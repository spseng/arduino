...
**************************************************************************/
 #if 1
        Serial.print("button status before decode: 0X");
        Serial.print(buttonStatus, HEX );
        Serial.print("  ");
        Serial.println( oldButtonStatus, HEX);
#endif  
    
    if (buttonStatus != oldButtonStatus) {

        oldButtonStatus = buttonStatus;
         phone_cmd=0; //test of phone_cmd
...
*/        
        ...
        if ((composite & (OP_STATUS | RANGE_STATUS)) == (OP_STATUS | RANGE_STATUS)) {
           ...
           //if reverse button pressed on car, run reverse        
        if((composite & REV_STATUS) == REV_STATUS){
            digitalWrite(REV_RELAY_DRV, 1);
            digitalWrite(RUN_RELAY_DRV, 1);
        }
        //otherwise turn off drive
        else {
            digitalWrite(REV_RELAY_DRV, 0);
        }
        
        //if parent presses STOP on phone (button 3)
        if (phone_cmd == 0x08){
            digitalWrite(REV_RELAY_DRV, 0);
            digitalWrite(RUN_RELAY_DRV, 0);
        }
        Serial.print(" phone command ");
        Serial.print(phone_cmd, HEX);


        //if parent presses REVERSE on phone (button down arrow)
        if (phone_cmd == 0x40){
            digitalWrite(REV_RELAY_DRV, 1);
            digitalWrite(RUN_RELAY_DRV, 1);
        }
